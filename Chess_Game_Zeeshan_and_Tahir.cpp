...
#define PAWN 1
#define KNIGHT 2
#define BISHOP 3   //#define to use create macro/constant value. 
#define ROOK 4
#define QUEEN 5
#define KING 7

using namespace std;
void gotoxy(int x, int y); //it is protoType
void set_data_highlight(int xx, int yy, int pp, bool uh);//it is protoType
void check_and_unhighlight();//it is protoType
bool end_game_or_not();//it is protoType
void tell_about_check(int, int);//it is protoType
...
public:
	...
	void greenStep() {
		...
		for (int i = 0; i < 5; i++) {                //for degree symbols
			cout << "\xB0";
		}
	}
	//Gray Step
...
public:
	...
	void printBlank(int cx, int cy) {
		if (cx % 2 == 0 && cy % 2 == 0) {            //If x is even, y is evem
			...
			int Y = 24 - (cy * 3) + 1;
			gotoxy(X, Y);
			cout << "\xDB\xDB"; //it prints like paint
		}
			...
			int Y = 24 - (cy * 3) + 1;
			gotoxy(X, Y);
			cout << "\xB0\xB0"; //it prints like dots
		}
	...
	void printPiece(int cx, int cy, int color, int pieceNum) {
		...
		int Y = 24 - (cy * 3) + 1;
		gotoxy(X, Y);
		if (pieceNum == PAWN) {  //piallla
			cout << "P";
		}
		else if (pieceNum == ROOK) {  //ele
			cout << "E";
		}
		else if (pieceNum == KNIGHT) {//horse
			cout << "H";
		}
		else if (pieceNum == BISHOP) {//Camel
			cout << "C";
		}
		...
		if (gameSquares[coord_x][coord_y].kill_him != whose_turn) { goto WRONG_PIECE; }
			...
			NEW_MOVE:
				...
				if (gameSquares[X][Y].is_highlight == 1) {
					...
					MyChessGame::data[X][Y].unhighlight_it = 0;
					gotoxy(5, 15);
...
void set_data_highlight(int xx, int yy, int pp, bool uh) {
	MyChessGame::data[xx][yy].x_coordinate = xx;
	MyChessGame::data[xx][yy].y_coordinate = yy;
	MyChessGame::data[xx][yy].kept_piece = pp;
	MyChessGame::data[xx][yy].unhighlight_it = uh;
}
//Check And Unhighlight the Highlighted Steps
void check_and_unhighlight() {
	for (int i = 1; i < 9; i++) {
		for (int j = 1; j < 9; j++) {
			if (MyChessGame::data[i][j].unhighlight_it == 1) {
				gameSquares[i][j].unhighlightStep(i, j);
				MyChessGame::data[i][j].unhighlight_it = 0;
			}
		}
	}
}
		...
		if (gameSquares[cx][cy].kill_him == 0) {
			...
			if (temp_x >= 8 || temp_y >= 8) {}
				...
				for (int i = cx + 1, j = cy; /*<No Condition>*/; i++, j++) {
					...
					if (gameSquares[i][j + 1].is_empty == 0 && gameSquares[i][j + 1].kill_him == 0) { break; }
					if (gameSquares[i][j + 1].is_empty == 0 && gameSquares[i][j + 1].kill_him == 1) { break; }
					if (gameSquares[i][j + 1].which_piece == KING && gameSquares[i][j + 1].kill_him == 1) {
						gotoxy(5, 15);
						HANDLE hConsole;
						hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
						SetConsoleTextAttribute(hConsole, 207);          //RED Colour Code = 207
